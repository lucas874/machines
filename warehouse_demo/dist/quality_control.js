...
const result_projection = (0, machine_check_1.projectCombineMachines)(warehouse_protocol_1.interfacing_swarms, warehouse_protocol_1.subs, "QCR");
if (result_projection.type == 'ERROR')
    throw new Error('error getting projection');
const projection = result_projection.data;
// Command map
const cMap = new Map();
cMap.set(warehouse_protocol_1.Events.report.type, (s, _) => {
    console.log("the newly built", s.self.modelName, " is", s.self.decision);
    return [warehouse_protocol_1.Events.report.make({ modelName: s.self.modelName, decision: s.self.decision })];
});
cMap.set(warehouse_protocol_1.Events.observing.type, (s, _) => {
    console.log("began observing");
    return [warehouse_protocol_1.Events.observing.make({})];
});
// Reaction map
const rMap = new Map();
const carReaction = {
...
rMap.set(warehouse_protocol_1.Events.car.type, carReaction);
const fMap = { commands: cMap, reactions: rMap, initialPayloadType: undefined };
// Extended machine
const [m3, i3] = warehouse_protocol_1.Composition.extendMachine("QCR", projection, warehouse_protocol_1.Events.allEvents, fMap);
const checkProjResult = (0, machine_check_1.checkComposedProjection)(warehouse_protocol_1.interfacing_swarms, warehouse_protocol_1.subs, "QCR", m3.createJSONForAnalysis(i3));
            ...
            for (var _d = true, machine_1 = __asyncValues(machine), machine_1_1; machine_1_1 = yield machine_1.next(), _a = machine_1_1.done, !_a; _d = true) {
                ...
                for (var c in s.commands()) {
                    if (c === 'observe') {
                        ...
                        }, (0, warehouse_protocol_1.getRandomInt)(2000, 5000));
                        break;
                    }
                    if (c === 'test') {
                        setTimeout(() => {

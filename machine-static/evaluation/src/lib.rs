...
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, PartialOrd)]
pub struct SubSizeSimpleProcessed {
    pub id: String,
    pub number_of_edges: usize,
    pub state_space_size: usize,
    pub efrac: f64,
...
pub fn read_sub_size_outputs_in_directory<T: DeserializeOwned>(directory: &Path) -> Result<Vec<T>> {
...
pub fn read_sub_size_ouput<T: DeserializeOwned>(path: &Path) -> Result<T> {
    let sub_size_output_string = std::fs::read_to_string(path)?;
    let sub_size_output = serde_json::from_str::<T>(&sub_size_output_string)?;
    Ok(sub_size_output)
}

// Used to read outputs generated by kmt23 vs compositional experiments
/* pub fn read_simple_sub_size_outputs_in_directory(directory: &Path) -> Result<Vec<SimpleProtoBenchmarkSubSizeOutput>> {
    let results: Result<Vec<DirEntry>, _> = WalkDir::new(directory).into_iter().collect();
    results?
        .into_iter()
        .filter(|entry| entry.file_type().is_file())
...
/* // Transform a file containing a sub size output to a SimpleProtoBenchmarkSubSizeOutput.
pub fn read_sub_size_ouput(path: &Path) -> Result<SimpleProtoBenchmarkSubSizeOutput> {
    let sub_size_output_string = std::fs::read_to_string(path)?;

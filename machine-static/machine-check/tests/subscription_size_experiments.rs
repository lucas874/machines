...
fn full_run_bench_sub_sizes_general() {
    ...
    for (_, bi) in interfacing_swarms_general.iter() {
        ...
        let subscriptions = match machine_core::overapproximated_well_formed_sub(swarms.clone(), SubscriptionsWrapped(subs.clone()), two_step_granularity.clone()) {
        ...
        let subscriptions = match machine_core::exact_well_formed_sub(swarms.clone(), SubscriptionsWrapped(subs.clone())) {
...
fn full_simple_run_bench_sub_sizes_general() {
    ...
    for input in inputs.iter() {
        ...
        let subscriptions_compositional_exact = match machine_core::exact_well_formed_sub(InterfacingProtocols(vec![input.proto.clone()]), SubscriptionsWrapped(subs.clone())) {
            DataResult::OK{data: subscriptions} => {
                Some(subscriptions) },
            DataResult::ERROR{ .. } => None,
        };
        ...
        let subscriptions_compositional_approx = match machine_core::overapproximated_well_formed_sub(InterfacingProtocols(vec![input.proto.clone()]), SubscriptionsWrapped(subs.clone()), two_step_granularity.clone()) {
            DataResult::OK{data: subscriptions} => {
                Some(subscriptions) },
            DataResult::ERROR{ .. } => None,
        };
